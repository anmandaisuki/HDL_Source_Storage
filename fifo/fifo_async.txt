...
意外とシンプルでfullとemptyを作成するときのポインタがレイテンシをもつ（この過程でグレイコードの変換生じる）部分以外はread側とwrite側で独立している。
そのためfull,emptyフラグの生成以外でグレイコードは関わらない。
...
assign full = (w_addr[ADDR_WIDTH]!=r_addr2[ADDR_WIDTH]) && (w_addr[ADDR_WIDTH-1:0]==r_addr2[ADDR_WIDTH-1:0]); <=このあたりはほとんど公式みたいなもの。
